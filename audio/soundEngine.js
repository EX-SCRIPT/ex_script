...
(function () {
  ...
  let pianoVoices = [];
  const NUM_PIANO_VOICES = 4;
  const NUM_PIANO_PARTIALS = 5;
  ...
  function buildPianoVoice(context, sharedNoiseSource, destination) {
    ...
    const partials = [];
    for (let k = 0; k < NUM_PIANO_PARTIALS; k++) {
      const oscillator = context.createOscillator();
      oscillator.type = 'sine';
      oscillator.frequency.value = 220 * (k + 1);
    ...
    hammerFilter.frequency.value = 1200;
    hammerFilter.Q.value = 1.2;

    const hammerGain = context.createGain();
    hammerGain.gain.value = 0;

    sharedNoiseSource.connect(hammerFilter);
    hammerFilter.connect(hammerGain);
    hammerGain.connect(voiceGain);

    return { partials, hammerFilter, hammerGain, voiceGain, panner };
  }
  ...
  function init() {
    ...
    pianoVoices = [];
    for (let i = 0; i < NUM_PIANO_VOICES; i++) {
      pianoVoices.push(buildPianoVoice(audioContext, noiseSource, masterGainNode));
    }
  ...
  function triggerPianoNoteAt(index, when, freqHz, peakGain) {
    ensureReady();
    ...
    for (let k = 0; k < voice.partials.length; k++) {
      const n = k + 1;
      const partial = voice.partials[k];
      const partialFreq = freqHz * n * Math.sqrt(1 + PIANO_INHARMONICITY * n * n);

      const freqParam = partial.oscillator.frequency;
      freqParam.cancelScheduledValues(when);
  ...
  function dampPianoVoiceAt(index, when, releaseTimeConstant) {
    ensureReady();
    const voice = pianoVoices[index];
    const release = releaseTimeConstant != null ? releaseTimeConstant : 0.045;
    for (const partial of voice.partials) {
      const gainParam = partial.gain.gain;
      gainParam.cancelScheduledValues(when);
      gainParam.setTargetAtTime(0, when, release);
    }
    const hammerParam = voice.hammerGain.gain;
    hammerParam.cancelScheduledValues(when);
    hammerParam.setTargetAtTime(0, when, 0.01);
  }
