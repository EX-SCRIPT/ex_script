...
(function () {
  ...
  const WHITE_REFERENCE_PERCENTILE = 0.94;
  ...
  const CONTEXT_MIN_WHITE_FRACTION = 0.10;
  ...
  const POROSITY_MIN_WHITE_FRACTION = 0.05;
  ...
  function computeConfidence(classified, whiteRef) {
    ...
    if (whiteCells.length < 1) return 0;
    ...
    score *= Math.max(0, 1 - clutterCells.length / 5);
  ...
  function analyzeFrame(videoEl) {
    ...
    const bandsWhiteRef = estimateWhiteRefFromBands([topBand, imageData, bottomBand]);
    const result = analyzeImageData(imageData, ANALYSIS_WIDTH, ANALYSIS_HEIGHT, bandsWhiteRef);
    ...
    if (rawConfidence > 0 && result.whiteRef > 0) {
      ...
      if (rawConfidence > 0) {
        const porosity = porosityOverColoredColumns(
          result.classified, [topBand, imageData, bottomBand], result.whiteRef,
          ANALYSIS_WIDTH, ANALYSIS_HEIGHT
        );
        if (porosity < POROSITY_MIN_WHITE_FRACTION) {
          rawConfidence = 0;
        }
      }
    }
  ...
  function estimateWhiteRefFromBands(bands) {
    const values = [];
    for (const band of bands) {
      const data = band.data;
      for (let i = 0; i < data.length; i += 16) {
        values.push(Math.max(data[i], data[i + 1], data[i + 2]) / 255);
      }
    }
    values.sort((a, b) => a - b);
    const idx = Math.min(values.length - 1, Math.floor(values.length * 0.92));
    return values[idx];
  }

  function porosityOverColoredColumns(classified, bands, whiteRef, width, bandHeight) {
    const coloredCells = classified.filter((c) => c.isColored);
    if (coloredCells.length === 0) return 1;
   ...
   */
  function analyzeImageData(imageData, width, height, whiteRefOverride) {
    const cells = sampleCells(imageData, width, height);
    const whiteRef = whiteRefOverride || estimateWhiteReference(cells);
  ...
  window.GridDetector = {
    ...
    porosityOverColoredColumns,
    estimateWhiteRefFromBands,
    reset,
